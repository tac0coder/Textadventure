...
message(
    ...
    0.05)
dynamite = item(5, 'dynamite', 'Blows stuff up in mine or explore modes',type='dynamite')
...
pickaxe = item(
    ...
    [
        'import time,os,thread','thread.run("run.py")', 'mining = open("static.txt","a+")', 'ores = []','os.system("start python run.py")',
        'while mining.read() == None:\n\ttime.sleep(3)\n\tores.append(percent("pickaxe tier 1"))\n\tos.system("clear")\n\tores_len = len(ores)\n\tprint(f"ores mined:{ores_len}  ore mined now:{ores[ores_len-1]}")','mining.truncate(0),mining.close()'
    ],'pickaxe')
miner = item(1, 'miner', 'Mines ores with \033[4mpickaxe\033[0m')
if input() == '':
  while True:
